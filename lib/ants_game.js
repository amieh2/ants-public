 ...
 */
class Insect {
  constructor(armor, place, isWaterSafe) {
    this._armor = armor;
    this._place = place;
    this._isWaterSafe = isWaterSafe;
  }
  
  get isWaterSafe() {
      return this._isWaterSafe();
  }
 ...
 */
class Place {
  constructor(isWater, name, exit, entrance) {
    this.name = name;
    this._bees = [];
    ...
    if(insect instanceof Bee) {
      ...
      return true;
       ...
       if (this._ant.canContain(insect)) {
           this._ant.hasAnt(insect);
            return true;
        } else if (insect.canContain(this._ant)) {
              insect.hasAnt(this._ant);
              return true;
        }
    }
    return false;
  ...
  getClosestBee(minDistance, maxDistance) {
   ...
   //Returns nearby bees, between the minDistance and the maxDistance ahead. 
  getClosestBees(minDistance, maxDistance) {
		var p = this;
		for(var dist = 0; p!==undefined && dist <= maxDistance; dist++)
		{
			if(dist >= minDistance && p.bees.length > 0) {
				return p.bees; //gets all the bees that meets the requirements
      }
			p = p.entrance;
		}
		return undefined; //no bee found
...
class AntColony {
  constructor(startingFood, numTunnels, tunnelLength, moatFrequency){
    ...
    this._beeEntrances = [];
    this._queenPlace = new Place(false,'Ant Queen');
    ...
    var prev, curr, typeName;
      ...
      this._places[tunnel] = [];
        ...
        curr = new Place(isWater,+'['+locationId+']', prev); //create new place with an exit that is the previous spot
  ...
  deployAnt(place, ant){
      if (place.isWater && !ant.isWaterSafe){
          return false; //could not place ant in water tunnels
      }
...
var Ants = {
  ...
  //hungry type
   Hungry : class extends Ant {
    ...
    act() {
      if(this._digesting > 0) {
          console.log(this + 'is digesting, ' + this._digesting + 'turns left.');
          this._digesting -= 1;
      } else {
          var target = this.place.getClosestBee(0,3);
        if(target){
   ...
   Fire : class extends Ant {
    ...
    reduceArmor(amount) {
    if(this._armor-amount <= 0) {
        var bees = this.place.getClosestBees(0,1);
        for(var i = 0; i < bees.length; i++) {
            bees[i].reduceArmor(3);
        }
    }    
    this._armor -= amount;
    if(this._armor <= 0){
  ...
  //ninja type
    ...
    act() {   
        for (var i = 0; i < this.place.bees.length; i++) {
        console.log(this + ' throws a rock at ' + this.place.bees[i])
        this.place.bees[i].reduceArmor(1);
      }     
    }
  },
  //bodyguard type
